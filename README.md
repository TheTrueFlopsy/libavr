...
TODO: Write a little about the philosophy of the library, i.e. not attempting
to replace or add layers on top of what already works well with just AVR Libc
and the hardware. For example, doing GPIO directly on registers doesn't seem
broken to me (would be even better if they kept the register names consistent
in the headers, though). Kinda the opposite of the Arduino approach in some
respects.
