...
with helper.InmHelper() as h:  # Create InmHelper with default message channel.
	...
	set_reg(h, nrf24x_reg(0x11), 0x04)
	get_reg_assert(h, nrf24x_reg(0x11), 0x04)
	
	# Set PRIM_RX bit in CONFIG register.
	set_reg(h, nrf24x_reg(0x00), 0x09)
	get_reg_assert(h, nrf24x_reg(0x00), 0x09)
	
	# Set PWR_UP bit in CONFIG register.
	set_reg(h, nrf24x_reg(0x00), 0x0b)
	time.sleep(0.01)  # Sleep for 10 milliseconds.
	get_reg_assert(h, nrf24x_reg(0x00), 0x0b)
	...
	while True:
		...
		# Read register STATUS/0x07 from nRF24 chip.
		nrf24x_st = get_reg(h, nrf24x_reg(0x07))
		...
		set_reg(h, nrf24x_reg(0x07), NRF24X_RX_DR)
		get_reg_assert(h, nrf24x_reg(0x07), 0x0e)
